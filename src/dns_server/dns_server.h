...
#define DNS_MAX_EVENTS 64                  // 减少事件数量以节省内存
#define DNS_CONN_BUFF_SIZE 2048            // 减少连接缓冲区大小
#define DNS_REQUEST_MAX_TIMEOUT 600        // 减少请求超时时间
#define DNS_PING_CHECK_INTERVAL (200)      // 增加检查间隔以节省电池
#define DNS_PING_RTT_CHECK_THRESHOLD (100 * 20) // 增加RTT阈值
#define SOCKET_IP_TOS (IPTOS_THROUGHPUT)   // 优化吞吐量而非低延迟
#define SOCKET_PRIORITY (0)                // 降低socket优先级
#define CACHE_AUTO_ENABLE_SIZE (1024 * 1024 * 2) // 降低自动启用缓存的大小
#define EXPIRED_DOMAIN_PREFETCH_TIME (3600 * 2)  // 减少过期域预取时间
#define DNS_MAX_DOMAIN_REFETCH_NUM 16      // 减少最大重新获取数量
#define DNS_SERVER_NEIGHBOR_CACHE_TIMEOUT 300      // 减少邻居缓存超时
#define DNS_SERVER_NEIGHBOR_CACHE_NOMAC_TIMEOUT 30 // 减少无MAC缓存超时
#define DNS_SERVER_NEIGHBOR_CACHE_MAX_NUM (1024 * 2) // 减少邻居缓存最大数量
#else
